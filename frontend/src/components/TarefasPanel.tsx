...
// TarefasPanel — conteudo de tarefas reutilizavel para ser embarcado na
// pagina /agenda como tab "Tarefas". Extrai a logica de TarefasPage
// sem o wrapper de pagina (h1, max-w-5xl, etc.) para poder ser composto.
//
// Fontes: agenda de todos os consultores + follow-ups vencidos + RNCs abertas
// Nao persiste estado no servidor — apenas client-side toggle (done/pending)
// Two-Base: nenhuma alteracao de valor R$ — so LOG/interacoes
...
const CONSULTORES_TAREFAS = ['MANU', 'LARISSA', 'DAIANE', 'JULIO', 'OUTROS'] as const;
...
const DATE_GROUPS: GroupConfig[] = [
  { key: 'atrasadas',      label: 'Atrasadas',      headerBg: 'bg-red-50',   headerText: 'text-red-900' },
  { key: 'hoje',           label: 'Hoje',           headerBg: 'bg-green-50', headerText: 'text-green-900' },
  { key: 'amanha',         label: 'Amanha',         headerBg: 'bg-blue-50',  headerText: 'text-blue-900' },
  { key: 'esta_semana',    label: 'Esta Semana',    headerBg: 'bg-gray-50',  headerText: 'text-gray-700' },
  { key: 'proxima_semana', label: 'Proxima Semana', headerBg: 'bg-gray-50',  headerText: 'text-gray-700' },
  { key: 'futuro',         label: 'Futuro',         headerBg: 'bg-gray-50',  headerText: 'text-gray-700' },
  { key: 'sem_data',       label: 'Sem Data',       headerBg: 'bg-gray-50',  headerText: 'text-gray-700' },
];
...
function assembleFromAgenda(items: AgendaItem[], consultor: string): Tarefa[] {
    ...
    >
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-md mx-4 overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          <div>
            <label className="block text-xs font-medium text-gray-700 mb-1.5" htmlFor="tp-nova-descricao">
              Descricao <span className="text-red-500">*</span>
            </label>
            <textarea
              id="tp-nova-descricao"
          ...
          <div>
            <label className="block text-xs font-medium text-gray-700 mb-1.5" htmlFor="tp-nova-cliente">
              Cliente
            </label>
            <input
              id="tp-nova-cliente"
          ...
          <div className="flex gap-3">
            <div className="flex-1">
              <label className="block text-xs font-medium text-gray-700 mb-1.5" htmlFor="tp-nova-due">
                Prazo
              </label>
              <input
                id="tp-nova-due"
            ...
            <div className="flex-1">
              <label className="block text-xs font-medium text-gray-700 mb-1.5" htmlFor="tp-nova-prio">
                Prioridade
              </label>
              <select
                id="tp-nova-prio"
...
// ---------------------------------------------------------------------------
// TarefasPanel — main export
// ---------------------------------------------------------------------------

export interface TarefasPanelRef {
  pendingTodayCount: number;
}

interface TarefasPanelProps {
  /** Callback chamado apos carregamento com contagem de pendentes hoje */
  onCountReady?: (count: number) => void;
}

export default function TarefasPanel({ onCountReady }: TarefasPanelProps) {
  useAuth();
  ...
  const loadTarefas = useCallback(async () => {
    ...
    setError(null);
    try {
      const agendaPromises = CONSULTORES_TAREFAS.map((c) =>
        fetchJson<AgendaConsultorResponse>(`/api/agenda/${encodeURIComponent(c)}`)
          .then((r) => assembleFromAgenda(r.itens.map((i) => ({ ...i })), c))
          .catch(() => [] as Tarefa[])
      );
  ...
  // Notificar parent com contagem de pendentes hoje
  useEffect(() => {
    if (!loading && onCountReady) {
      const now = new Date();
      const today = new Date(now.getFullYear(), now.getMonth(), now.getDate());
  ...
  const counts = useMemo(() => {
    ...
    return {
      ...
      next7: next7.length,
      urgentes: pending.filter((t) => t.raw_priority_num <= 3).length,
      followups: pending.filter((t) => t.tipo === 'FOLLOWUP').length,
      rncs: pending.filter((t) => t.tipo === 'RNC').length,
      concluidas: tarefas.filter((t) => t.status === 'done').length,
    };
  }, [tarefas]);

  const filtered = useMemo(() => {
    switch (activeFilter) {
      case 'urgentes':   return tarefas.filter((t) => t.status === 'pending' && t.raw_priority_num <= 3);
      case 'followups':  return tarefas.filter((t) => t.status === 'pending' && t.tipo === 'FOLLOWUP');
      case 'rncs':       return tarefas.filter((t) => t.status === 'pending' && t.tipo === 'RNC');
      case 'concluidas': return tarefas.filter((t) => t.status === 'done');
      default:           return tarefas.filter((t) => t.status !== 'done');
    }
  ...
  return (
    <div className="space-y-4">
      {/* Acoes: Nova Tarefa + botao reload */}
