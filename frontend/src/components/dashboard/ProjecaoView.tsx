...
// ---------------------------------------------------------------------------
// ProjecaoView — Projeção Comercial 2026 como componente puro
// Usado como tab interna do Dashboard (/) e como standalone em /projecao
// ---------------------------------------------------------------------------
...
export function ProjecaoView() {
  ...
  const meta2026 = META_2026_FALLBACK;
  ...
  return (
    <div className="space-y-6">
      {/* Page heading */}
      <div>
        <h2 className="text-lg sm:text-xl font-bold text-gray-900">Projecao Comercial 2026</h2>
    ...
    <div className="overflow-x-auto scrollbar-thin">
      ...
      >
        {[0, 0.25, 0.5, 0.75, 1].map((frac) => {
          ...
          return (
            <g key={frac}>
              <line x1={36} y1={y} x2={CHART_W} y2={y} stroke="#F3F4F6" strokeWidth={1} />
              <text x={34} y={y + 3} textAnchor="end" fontSize="9" fill="#9CA3AF">
                {labelStr}
              </text>
            </g>
          );
        ...
        {data.map((d, i) => {
          ...
          const barColor = pct >= 75 ? '#00B050' : pct >= 50 ? '#FFC000' : '#FF4500';
          ...
          return (
            <g key={d.mes_referencia}>
              <rect x={x} y={14 + CHART_H - metaH} width={BAR_W} height={metaH} fill="#E5E7EB" rx={3} />
              <rect x={x + BAR_W + GAP} y={14 + CHART_H - realH} width={BAR_W} height={realH} fill={barColor} rx={3} />
              <text x={x + BAR_W + GAP / 2} y={14 + CHART_H + 14} textAnchor="middle" fontSize="9" fontWeight="500" fill="#6B7280">
                {mesLabel}
              </text>
              {pct > 0 && realH > 12 && (
                <text x={x + BAR_W + GAP + BAR_W / 2} y={14 + CHART_H - realH - 4} textAnchor="middle" fontSize="8" fontWeight="700" fill={barColor}>
                  {pct.toFixed(0)}%
                </text>
              )}
            </g>
        ...
        <g>
          ...
          <text x={CHART_W - 115} y={12} fontSize="9" fill="#6B7280">Meta</text>
          <rect x={CHART_W - 85} y={4} width={12} height={8} fill="#00B050" rx={2} />
          <text x={CHART_W - 70} y={12} fontSize="9" fill="#6B7280">Realizado</text>
        </g>
      </svg>
    </div>
  );
