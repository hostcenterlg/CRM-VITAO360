...
//
// Rota canonica do manual. /docs (rota antiga) redireciona para ca — o nome
// /docs colidia com vercel.json raiz (que rota /docs para o backend FastAPI),
// resultando em 404 cacheado pelo Vercel no deploy do frontend.
// ---------------------------------------------------------------------------
  ...
  return (
    <div className={`border rounded-xl overflow-hidden transition-all ${open ? 'border-green-200 shadow-sm' : 'border-gray-200'}`}>
      ...
      >
        ...
        <svg
          className={`w-4 h-4 flex-shrink-0 transition-transform text-gray-500 ${open ? 'rotate-180' : ''}`}
...
function ScreenPlaceholder({ label }: { label: string }) {
  return (
    <div className="w-full rounded-lg border-2 border-dashed border-gray-200 bg-gray-50 py-6 flex flex-col items-center justify-center text-gray-500 text-xs gap-1">
      ...
      <div className="flex gap-2 mb-6">
        {(Object.entries(ROLE_CONFIG) as [Role, typeof ROLE_CONFIG[Role]][]).map(([role, c]) => (
          ...
          >
            ...
            <div className="text-xs opacity-70 mt-0.5">{c.subtitle}</div>
          </button>
        ))}
      </div>
